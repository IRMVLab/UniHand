...
class SAM3DBodyEstimator:
    ...
    def process_one_image(
        ...
        if inference_type == "full":
            pose_output, batch_lhand, batch_rhand, lhand_output, rhand_output = outputs
        else:
            pose_output = outputs
            rhand_output = None
        ...
        # Right-hand 2D from hand decoder (hand crop space) for accurate crop->original transform
        rhand_kpt_crop = None
        if inference_type == "full" and rhand_output is not None:
            _r2d = rhand_output["mhr_hand"]["pred_keypoints_2d"]  # [1, N, 70, 2]
            _r2d = recursive_to(_r2d, "cpu")
            rhand_kpt_crop = np.asarray(_r2d.cpu().numpy() if hasattr(_r2d, "numpy") else _r2d, dtype=np.float64)
        all_out = []
        for idx in range(batch["img"].shape[1]):
            ...
            if rhand_kpt_crop is not None:
                # Right-hand 21 keypoints in hand crop space (MHR indices 21:42)
                # rhand_kpt_crop shape: (N, 70, 2) or (1, N, 70, 2)
                rk = rhand_kpt_crop
                if rk.ndim == 4:
                    rk = rk[0, idx, 21:42, :]
                else:
                    rk = rk[idx, 21:42, :]
                entry["pred_keypoints_2d_rhand_crop"] = np.asarray(rk, dtype=np.float64)
